...
module.exports = (function() { 
	...
	function tabClick ( d, i, ele ) {
		...
		if ( d.name === 'plus' ) {		//	add a tab?
			var s = '' + d.tabs.data.tabs.data.length;
		...
		indicateTabSelected ( d.tabs.data, i );
	...
	function indicateTabSelected ( tabsData, iTab ) {
		var td = tabsData.tabs.data[iTab];
		var tg = d3.select ( '#' + td.eleId );
		var lines = tg.selectAll ( 'line' ).nodes();
		d3.select ( lines[0] )		//	top
			.attr ( 'stroke', 'white' );
...
//		console.log ( sW + ' tabEleId: ' + tabEleId + '  n: ' + n + '  data.type: ' + s.nodes()[0].__data__.type );
//	}	//	tabDataName()
...
//	See createTabData().
	...
	Tabs.prototype.addTab = function ( o ) {
		...
		//	content - 
		//		There may be multiple tabs and their content being added here. For example,
		//		when tabs' parent is split.
		//		So first, remove panelData from tsd.childData if it is already there.
		...
		tab.each ( function ( d, i, g ) {	//	content 
			...
			if ( d.panel ) {		//	recreating these tabs because of, for example, adding a splitter?
				panelData = d.panel.data;
				panelData.baseData = [];
			} else {
				panelData = tsd.panelSvc.createPanelData ( { x: 		-0.5,  
															 y: 		-0.5, 
															 w: 		tsd.w, 
															 h: 		tsd.h - tsd.tabH, 
															 name: 		'tab-content', 
				...
				panelData.eleId = d.eleId + '-pnl';		//	Normally the parent panel sets this (in addControl()).  But 
			...
			d.panel = tsd.panelSvc.createPanel ( tsd.content, tsd.childData, false );
	...
	Tabs.prototype.rmvTab = function ( panel ) {
		...
		var tab = tsd.tabs.g.selectAll ( 'g' )
			...
			.exit()
			.each ( function ( d ) { 
				console.log ( sW + ' - tab - old data: ' + d.eleId ); 
			} )
			.remove();
	...
	TabsData.prototype.listProperties = function() {		//	called by showPropertiesBoard() 
		...
		for ( var key in this ) {
			...
			displayName = key;
			switch ( key ) {
				case 'tabs': 		displayName = 'tab text';		
...
//				console.log ( sW + ': - g - new data: ' + d.name ); 
		...
		//	Tabs.  But the contents of just one tab visible at a time.  Generally,
		//	all tabs (what is clicked on to change the tab viewed) are all visible
		//	- across the bottom, for now - the "tabs".
		//
		//	So, two areas (two <g>) -
		//
		//		content 			where the selected tab is shown - width of the tabs
		//							control - height of the tabs control minus the height 
		//							of the tabs
		//
		//		tabs 				what the user clicks on to select different content - 
		//							height determined by the font used for the text that 
		//							is the name/title of the tab and the content 
		//							is shown when the tab is selected as the content to
		//							be viewed
		//
		//	There is always at least one tab - even when no content for it is defined
		//	in which case the text will be a + (plus sign) and when the user clicks on 
		...
		//			a new tab is added
		//
		//			the text is blank - the user can click there to enter the text for the
		//			name/title of the content for that tab 
		//
		//	A new tabs control will therefore have one tab that runs the width of 
		//	the control and displays a simple + sign. The content area will be gray, possibly
		//	with some indication (text) that the user may drag a panel there.

		tabs.data.content = ctrlEles.append ( 'g' )
			.attr ( 'id',        function ( d, i ) { return d.eleId + '-content'; } )
			.attr ( 'transform', function ( d, i ) { return 'translate(' + 0 + ',' + 0 + ')'; } );

		tabs.data.tabH =   uLabel.measureText ( panel, tabs.data.ff, tabs.data.fs, '^j' ).h 
						 + uc.TABS_TABS_TAHA
						 + uc.TABS_TABS_EAH;

		tabs.data.content.append ( 'rect' )
			.attr ( 'id',     function ( d, i ) { return d.eleId + '-content-rect'; } )
			.attr ( 'width',  function ( d, i ) { return d.w; } )
			.attr ( 'height', function ( d, i ) { return d.h - d.tabH; } )
			.attr ( 'class',  function ( d, i ) { return d.classEmptyContent; } );

		tabs.data.tabs.g = ctrlEles.append ( 'g' )
			.attr ( 'id',        function ( d, i ) { return d.eleId + '-tabs'; } )
			.attr ( 'transform', function ( d, i ) { return 'translate(' + 0 + ',' 
																		 + (d.h - d.tabH) + ')'; } );

		if ( tabs.data.tabs.data.length === 0 ) {
			tabs.addTab ( { name: 	'plus',				//	commented out to test/debug one tab
							text: 	'+' } );			//	at a time
