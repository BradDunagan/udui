...
//	function getLoadSpec ( sW, uduiId, dlg, storeId, storeName ) {		obsolete  -  see uSL.getLoadSpec()
//		return { 
//			sC: 		sW, 
//			itemName: 	{
...
//	}	//	getLoadSpec()
	...
	//	var o =     getLoadSpec ( sW, args.udui, dlg, uc.NEW_LABEL_DLG_STORE_ID, uc.NEW_LABEL_DLG_STORE_NAME );
	//	var o =     getLoadSpec ( sW, args.udui, dlg, uc.NEW_LABEL_DLG_STORE_ID, uc.NEW_LABEL_DLG_STORE_NAME );
		...
		dlg.data = uPanel.createAppPanelData ( { x: 		x + uc.OFFS_4_1_PIX_LINE, 
		...
		dlg.panel.addControl ( uLabel.createLabelData ( { x: 20, y: 20, w: 60, h: 15, name:  		'lblLabelName',
																					  text:  		'Label Name:',
																					  vertAlign: 	'top',
																					  horzAlign: 	'left' } ) );
		dlg.panel.addControl ( uInput.createInputData ( { x: 80, y: 20, w: 80, h: 15, name:  'edtLabelName',
																					  value: args.label.data.name } ) );

		dlg.panel.addControl ( uLabel.createLabelData ( { x: 20, y: 40, w: 60, h: 15, name:  		'lblLabelText',
														  							  text:  		'Label Text:',
																					  vertAlign: 	'top',
																					  horzAlign: 	'left' } ) );
		dlg.panel.addControl ( uInput.createInputData ( { x: 80, y: 40, w: 80, h: 15, name:  'edtLabelText',
																					  value: args.label.data.text } ) );

		dlg.panel.addControl ( uLabel.createLabelData ( { x: 20, y: 60, w: 60, h: 15, name:  		'lblLabelFontName',
														  							  text:  		'Font Name:',
																					  vertAlign: 	'top',
																					  horzAlign: 	'left' } ) );
		dlg.panel.addControl ( uInput.createInputData ( { x: 80, y: 60, w: 80, h: 15, name:  'edtLabelFontName',
																					  value: args.label.data.ff } ) );

		dlg.panel.addControl ( uLabel.createLabelData ( { x: 20, y: 80, w: 60, h: 15, name:  		'lblLabelFontSize',
														  							  text:  		'Font Size:',
																					  vertAlign: 	'top',
																					  horzAlign: 	'left' } ) );
		dlg.panel.addControl ( uInput.createInputData ( { x: 80, y: 80, w: 80, h: 15, name:  'edtLabelFontSize',
																					  value: '' + args.label.data.fs } ) );

		dlg.panel.addControl ( uButton.createButtonData ( { x: 20, y: 100, w: 60, h: 20, name: 	'btnOK',
														    							 text: 	'OK',
														    							 cb: 	onOK } ) );

		dlg.panel.addControl ( uButton.createButtonData ( { x: 90, y: 100, w: 60, h: 20, name: 	'btnCancel',
														    							 text: 	'Cancel',
														    							 cb: 	onCancel } ) );
	};	//	newLabelDialog()
	...
	//	var o = 	getLoadSpec ( sW, args.udui, dlg, uc.GRID_DLG_STORE_ID, uc.GRID_DLG_STORE_NAME );
		var uSL    = uc.storeLoadSvc;		//	2018-May-07
		var uPanel = uc.panelSvc;			//	2018-May-07
		var o = uSL.getLoadSpec ( { sW: 		sW, 
									uduiId: 	args.uduiId, 
		...
		dlg.panel.addControl ( uCheckbox.createCheckBoxData ( { x: 20, y: 20, w: 60, h: 15, name:  'chkEnabled',
															  								text:  'Enabled',
														  									value: args.forPanel.data.grid.isEnabled } ) );

		dlg.panel.addControl ( uCheckbox.createCheckBoxData ( { x: 20, y: 20, w: 60, h: 15, name:  'chkVisible',
														  									text:  'Visible',
														  									value: args.forPanel.data.grid.isVisible } ) );

		dlg.panel.addControl ( uLabel.createLabelData ( { x: 20, y: 20, w: 60, h: 15, name:  'lblXSpacing',
														  							  text:  'X Spacing:' } ) );
		dlg.panel.addControl ( uInput.createInputData ( { x: 80, y: 20, w: 80, h: 15, name:  'edtXSpacing',
																					  value: '' + args.forPanel.data.grid.spaceX } ) );

		dlg.panel.addControl ( uLabel.createLabelData ( { x: 20, y: 20, w: 60, h: 15, name:  'lblYSpacing',
														  							  text:  'Y Spacing:' } ) );
		dlg.panel.addControl ( uInput.createInputData ( { x: 80, y: 20, w: 80, h: 15, name:  'edtYSpacing',
																					  value: '' + args.forPanel.data.grid.spaceY } ) );

		dlg.panel.addControl ( uButton.createButtonData ( { x: 20, y: 60, w: 60, h: 20, name: 	'btnOK',
														    							text: 	'OK',
														    							cb: 	onOK } ) );

		dlg.panel.addControl ( uButton.createButtonData ( { x: 80, y: 60, w: 60, h: 20, name: 	'btnCancel',
														    							text: 	'Cancel',
														    							cb: 	onCancel } ) );
	};	//	showGridDialog()
	...
	//	var o = 	getLoadSpec ( sW, args.udui, dlg, uc.PROPERTIES_DLG_STORE_ID, uc.PROPERTIES_DLG_STORE_NAME );
		var uSL    = uc.storeLoadSvc;		//	2018-May-07
		var uPanel = uc.panelSvc;			//	2018-May-07
		var o = uSL.getLoadSpec ( { sW: 		sW, 
									udui: 		args.udui, 
		...
		panelD = uPanel.createPanelData ( { x: 			10 + uc.OFFS_4_1_PIX_LINE,
										  	y: 			10 + uc.OFFS_4_1_PIX_LINE, 
										  	w: 			panelW, 
										  	h: 			140, 
										  	name: 		'pnlProps', 
										  	clickCB: 	uc.rootPanelClick } );
		...
		dlg.panel.addControl ( uButton.createButtonData ( { x: 		20,
														    ...
														    text: 	'OK',
														    cb: 	onOK } ) );
		dlg.panel.addControl ( uButton.createButtonData ( { x: 		100,
														    y: 		160,
														    w: 		60,  
														    h: 		20, 
														    name: 	'btnCancel',
	...
	svc.showLoginDialog = function ( args ) {
		...
		dlg.panel.addControl ( uButton.createButtonData ( { x: 30,     y: h - 50, w: 60, h: 20, 
															name: 	'btnOK',
														    text: 	'OK',
														    cb: 	args.onOK } ) );

		dlg.panel.addControl ( uButton.createButtonData ( { x: w - 90, y: h - 50, w: 60, h: 20, 
															name: 	'btnCancel',
														    text: 	'Cancel',
														    cb: 	args.onCancel } ) );
		return dlg;
