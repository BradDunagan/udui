...
module.exports = (function() { 
	...
	function mouseOverItem ( d, i, ele ) {
	//	var sW = serviceId + ' mouseOverItem()';
		...
		ld.mouseOver = { g: this, d: d, i: i, ele: ele };
	}	//	mouseOverItem()

	function mouseLeaveItem ( d, i, ele ) {
	...
	function clickItem ( d, i, ele ) {
		var sW = serviceId + ' clickItem()';
		var ld = d.ld, g;
		if ( event.altKey ) {
			console.log ( sW + ':  altKey' );
			return;
		}

		console.log ( sW + '  Item text: ' + d.text );
		if ( ! ld.wasScrolling ) {
			if ( ld.itemSelected ) {
...
//		d.dragStartY = d3.event.y;
	...
	function mouseMove ( d, i, ele ) {
		...
		if ( ! op )
			return;
		op.updateXY ( event.pageX, event.pageY );		//	uCD.mouseDown() should have set up uc.mouseOp
		var dy = op.y - op.y0;
		...
		g.select ( '#' + d.eleId + '-items-rect' )
			.attr ( 'y', function ( d, i ) { return -d.items.y; } );
	}	//	mouseMove()
...
//		console.log ( sW + '  panel: ' + d.name + '  clientX Y: ' + event.clientX + '  ' + event.clientY );
		event.stopPropagation();
		if ( d.clickDisabled ) {
	...
	function sizeItemsRectW ( d, i ) {
		return d.w - (1 * uc.LIST_BORDER_WIDTH);
	}	//	sizeItemsRectW()

	function sizeItemsRectH ( d, i ) {
		return d.h - (1 * uc.LIST_BORDER_WIDTH);
	}	//	sizeItemsRectH()
	...
	//	this.fs   = 10;						//	font size, pixels
		this.fs   = 12;						//	font size, pixels	2018-Apr-21
...
//	ListData.prototype.listProperties = function() {		//	called by showPropertiesBoard() 
	function ListData_listProperties() {		//	called by showPropertiesBoard() 
...
//	ListData.prototype.update = function() {
	function ListData_update() {
		...
		var	is = lg.selectAll ( 'g' ) 							//	g for each item ...
			.data ( ld.itemData, function ( d ) {
				return d.id || (d.id = ++ld.itemNextId);
			} );

		var ig = is.enter()										//	... add new item g 
			.append ( 'g' )
			.each ( function ( d ) { d.ld = ld; } )
			.attr ( 'id',     function ( d, i ) { 
				var eleId = ld.eleId + '-rect-item-' + i;
//				console.log ( sW + ' - appending list item - eleId: ' + eleId );
				d.g = this;
				return eleId; 
			} )
			.attr ( 'transform', function ( d, i ) { 
...
//	ListData.prototype.jsonize = function() {
	function ListData_jsonize() {						//	2018-May-12
		var sW = serviceId + ' ListData.jsonize()';
		var ld = this;

		//	Return a JSON form of the list. Make sure each list item includes
		//	its element Id.

		var whiteList = [ 'id', 'textId', 'text' ];
		...
		function replacer ( key, value ) {
			...
			if ( Array.isArray ( this ) )			//	key will be the string form of the index
				return value;
			if ( key === 'g' )
				return value.id;					//	The <g>'s element Id.
			if ( whiteList.includes ( key ) )
				return value;
			return undefined;
		}	//	replacer()

		return JSON.stringify ( ld.itemData, replacer );
	}	//	ListData.prototype.jsonize()

//	ListData.prototype.getItemEleId = function() {
	function ListData_getItemEleId ( itemTextId ) {						//	2018-May-12
		var sW = serviceId + ' ListData.getItemEleId()';
		var ld = this;

		var item = ld.itemData.find ( function ( item ) {
			return item.textId === itemTextId;
		} );

		if ( ! uc.isDefined ( item ) )
			return 'not found';
			
		return item.g.id;
	}	//	ListData.prototype.getItemEleId()

	svc.createListData = function ( x, y, w, h, name, cb ) {
		...
		if ( ListData.prototype.constructor.name === 'ListData' ) {
			...
			ListData.prototype.jsonize = ListData_jsonize;
			ListData.prototype.getItemEleId = ListData_getItemEleId;
		}

		return new ListData ( x, y, w, h, name, cb );
	};	//	svc.createListData()
