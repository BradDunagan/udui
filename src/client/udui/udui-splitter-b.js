...
//		console.log ( sW );
//		console.log ( sW + ' d.x: ' + d.x + '   d3.event.x: ' + d3.event.x + '   d3.event.dx: ' + d3.event.dx );
	//	if ( d.vh === 'horz' )
	//		d3.select ( this )
...
//	svc.createSplitterData = function ( x, y, w, h, name, vh ) {
	...
	SplitterData.prototype.move = function ( g, x, y, dx, dy ) {
		...
		if ( this.vh === 'horz' )
			d3.select ( g )
				.attr ( 'transform', function ( d, i ) { return 'translate(' + (d.x = x) + ',' + d.y + ')'; } );
		if ( this.vh === 'vert' )
			d3.select ( g )
				.attr ( 'transform', function ( d, i ) { return 'translate(' + d.x + ',' + (d.y = y) + ')'; } );
//		console.log ( sW + ' d.y: ' + d.y );
	...
	//		.data ( panel.data.childData.data, function ( d ) { 
		...
		var g = s
			...
			.enter()
			.each ( function ( d ) { console.log ( 'defineSplitter() - g - new data: ' + d.name ); } )
			.append ( 'g' )
